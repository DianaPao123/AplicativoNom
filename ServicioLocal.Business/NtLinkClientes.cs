...
namespace ServicioLocal.Business
{
    public class NtLinkClientes : NtLinkBusiness
    {
        ...
        //-------------------------------------------------------------
        public List<SubContratacion> GetListSubContratacion( int idCliente)
        {
            List<SubContratacion> result = new List<SubContratacion>();
            try
            {
                using (var db = new NtLinkLocalServiceEntities())
                {
                   
                   result = db.SubContratacion.Where( p => p.IdCliente == idCliente ).ToList();
        ...
        public bool EliminarCliente(clientes cliente)
        {
            try
            {
                ...
                using (var db = new NtLinkLocalServiceEntities())
                {
                    ...
                    if (existe)
                        throw new FaultException("El empleado tiene recibos capturados, no es posible eliminar");
        ...
        public bool EliminarSubcontratacion(int idcliente )
        {
            try
            {
                ...
                using (var db = new NtLinkLocalServiceEntities())
                {
                    var existe = db.SubContratacion.Where(p => p.IdCliente == idcliente).ToList();
                    
                    // Comprobamos si todo es correcto
                    foreach (SubContratacion sub in existe)
                    {

                        db.SubContratacion.DeleteObject(sub);
                    }
                    db.SaveChanges();
                    return true;
                }
        ...
        public bool SaveDatosNomina(DatosNomina datos)
        {
            try
            {
                ...
                using (var db = new NtLinkLocalServiceEntities())
                {
                    ...
                    {
                        var cliente = db.clientes.FirstOrDefault(p => p.idCliente == datos.IdCliente);
                        var numero = db.clientes.Count(p => p.idempresa == cliente.idempresa && p.idCliente != cliente.idCliente);
                        numero++;
                        datos.NoEmpleado = numero.ToString().PadLeft(6, '0');
        ...
        public bool SaveSubcontratacioNomina(SubContratacion datos)
        {
            try
            {
                ...
                using (var db = new NtLinkLocalServiceEntities())
                {
                        db.SubContratacion.AddObject(datos);
                      db.SaveChanges();
                    return true;
                }
            }
        ...
        private bool Validar(clientes e)
        {
            //TODO: Validar los campos requeridos y generar excepcion
            {
                ...
                Regex regCurp = new Regex("[A-Z][A,E,I,O,U,X][A-Z]{2}[0-9]{2}[0-1][0-9][0-3][0-9][M,H][A-Z]{2}[B,C,D,F,G,H,J,K,L,M,N,Ñ,P,Q,R,S,T,V,W,X,Y,Z]{3}[0-9,A-Z][0-9]");
                if (!regCurp.IsMatch(e.CURP))
                {
                    throw new FaultException<ApplicationException>(new ApplicationException("El CURP es inválido"), "El CURP es inválido");
                }
            }
            return true;
        }
